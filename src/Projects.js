...
function MetaCollab(props){
    ...
    if(collabExists){
        return(
            <DesignerInfo projectDesigner = {props.projectDesigner} designerURL={props.designerURL} />
        );
    } else {
        return '';
    }
...
function Project(props){
    ...
    const mapTags = props.projectTags.map((projectTag)=>{
        return(
            <li key={String(Math.min(Math.random()+ 100)) + projectTag.toString()}>{projectTag}</li>
        )})
              ...
              <div className="project__meta">
                  <div className="project__meta--basic">
                    <span className="project__meta--company">
                        ...
                        <div className="btn-group">
                            <MetaProjectRepo projectURL={props.projectURL} projectRepo = {props.projectRepo} />
                        </div>
                    </span>
                    <span className="project__meta--role">Role: <i>{props.projectRole}</i></span> 
                  </div>
                  <MetaCollab projectDesigner = {props.projectDesigner} designerURL={props.designerURL} />
              </div>
      ...
      return(
        <section id="work" className="section-padding">
          ...
          <p>Add Ziliute and TRRS </p>
            ...
            projectRepo = 'https://github.com/michelegrace/swrtheme'>
            </Project>
            <Project 
            src={tbn}
            projectDesigner = ''
            ...
            designerURL = ''
            projectDesc = 'A single page website for Barlogic, a company offering a variety of products to help bar owners and restaurateurs run their businesses.' 
            projectTags = {['Bootstrap','Node/NPM', 'Node/NPM', 'SCSS', 'SVG', 'Wordpress']} 
